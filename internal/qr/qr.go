...
func CreatePrompPayQR() string {
	formatedPromptPayPhoneNumber := "0066809710099"
	...
	promptPayPhoneNumberSubField := qrPromptPayField{id: "01", name: "Prompt phone number", length: 13, value: formatedPromptPayPhoneNumber}
	...
	qr := serialize(payloadFormatIndicator) + serialize(poiMethod) + serialize(merchantAccountInformation) + serialize(country) + serialize(currency)
...
func serialize(field qrPromptPayField) string {
	...
	if field.subField == nil {
		output = field.id + formatFieldLengthStr(field.length) + field.value
	} else {
		var subFieldOutput string
		for _, sub := range field.subField {
			subFieldOutput = subFieldOutput + sub.id + formatFieldLengthStr(sub.length) + sub.value
		}
		output = field.id + formatFieldLengthStr(field.length) + subFieldOutput
	}
	return output
}
